...
while i < len(lines):
    ...
    if GenerationNo == 1:
        ...
        for notused in xrange(popsize):
            variables = [float(x) for x in lines[i].split("\n")[0].split("|**|")[0].split(" ") if
                         x != ""]  # Get variables on line i before |**| (old population)
    ...
    if GenerationNo == int(ngener):
        ...
        for notused in xrange(popsize):
            variables = [float(x) for x in lines[i].split("\n")[0].split("|**|")[1].split(" ") if
                         x != ""]  # Get variables on line i after |**| (old population)
...
while i < len(lines):
    ...
    if lines[i] == "\n":
        ...
        if i >= len(lines):  # break if the loop is completed
            break
    Objs1 = [float(x) for x in lines[i].split("\n")[0].split("|**|")[0].split(" ") if x != ""][
            prmtrno:-2]  # Get Objective functions on line i before |**| (old population)
    Objs2 = [float(x) for x in lines[i].split("\n")[0].split("|**|")[1].split(" ") if x != ""][
            prmtrno:-2]  # Get Objective functions on line i after |**| (mate population)
