...
class nsga2:
    def __init__(self, SWATtxtinoutFolderDirectory):
        ...
        if "linux" in sys.platform.lower() or "darwin" in sys.platform.lower():
            ...
            f = codecs.open('{:}/nsga2_mid.cmd'.format(SWATtxtinoutFolderDirectory), 'w', encoding = 'utf-8')
            f.write(
        ...
        f = codecs.open(nsga2def, "r", encoding = 'utf-8')
        lines = f.readlines()
        ...
        # Read 'nsga2_par.def'
        f = codecs.open(nsga2pardef, "r", encoding = 'utf-8')
        lines = f.readlines()
        nchrom = 0
        ...
        for i in range(nchrom):
            ...
            lim_b.append([float(lines[i + 1].split()[1]),
                          float(lines[i + 1].split()[2])])  # lower & the upper limits of the %d variable\n"%(i+1)
        pmut_b = pmutprp * 1.0 / chrom  # the mutation probability for binary strings (between 0 and 1.0/chrom)
        f.close()

        # Read 'observed_rch.txt'
        f = codecs.open(observed_rch, "r", encoding = 'utf-8')
        lines = f.readlines()
        ...
        for i in range(0, len(lines)):
            try:
                ...
                sys.exit("ERROR: check the 'observed_rch.txt' file (the gage on line:" + str(i) + ")")
    ...
    def CreateInitialPopulation(self):
        ...
        if not os.path.exists(self.SWATdir + "/NSGA2.OUT"):
            os.makedirs(self.SWATdir + "/NSGA2.OUT")

        # Define the the initital population
        if self.ReadMFrmOut == 1:  # Read Last population from output.out
            # Read output.out
            f = codecs.open(self.SWATdir + "/NSGA2.OUT/output.out", "r", encoding = 'utf-8')
            lines = f.readlines()
            ...
            SWATutilities.CalculateObjectiveFunctions(old_pop_ptr, self.Outlet_Obsdata, self.FuncOpt, self.FuncOptAvr,
                                                      self.parname, "Previous NSGA-II run Last Population",
                                                      self.SWATdir)
            ...
            for i in range(self.M):
                for j in range(self.nchrom):
                    ...
                    # /*Function Calculaiton*/
            SWATutilities.CalculateObjectiveFunctions(InitialLHSpop, self.Outlet_Obsdata, self.FuncOpt, self.FuncOptAvr,
                                                      self.parname, "LHS", self.SWATdir)
    ...
    def CreateChildPopulation(self):
        ...
        if (self.optype == 1):  # /*Binary Cross-over*/
            self.ncross = nsga2utilities.crossover(self.new_pop_ptr, self.mate_pop_ptr,
                                                   self.warmup_random, self.pcross, self.ncross)

        if (self.optype == 2):  # /*Binary Uniform Cross-over*/
            self.ncross = nsga2utilities.unicross(self.new_pop_ptr, self.mate_pop_ptr,
                                                  self.warmup_random, self.pcross, self.ncross)
        # Mutation
    ...
    def CreateParentPopulation(self, generationNo):
        nsga2utilities.CreateMatePopFromNewandOldPops(self.old_pop_ptr, self.new_pop_ptr, self.mate_pop_ptr,
