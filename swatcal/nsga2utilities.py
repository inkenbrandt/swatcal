...
def reverse_decode(pop_ptr, vlen, lim_b):  # Calculates binary (genes) values from the xbin values
    ...
    while i < popsize:
        ...
        while m < nchrom:
            sum_ = int(
                (pop_ptr['ind'][i]['xbin'][m] - lim_b[m][0]) * (pow(2, vlen[m]) - 1) / (lim_b[m][1] - lim_b[m][0]))
            k = vlen[m] - 1
            while k > -1:
                if sum_ % 2 == 1:
...
def indzeros(
        chrom):  # Creates individual with "genes", "rank" and "cub_len" with zero values. genes= a parameter set in a binary form, cub_len=crowding distance
    indvdl = {}
    indvdl["genes"] = numpy.zeros(chrom, int)  # /*bianry chromosome*/
    indvdl["rank"] = 0  # /*Rank of the individual*/
    indvdl["cub_len"] = 0.0  # /*crowding distance of the individual*/
...
def Selection(old_pop_ptr, pop2_ptr,
              warmup_random):  # nselect() in deb's c code #select solutions from old population to the mate population. All members on mate population updated.
    ...
    for n in range(popsize):
        ...
        if rnd <= 0:
            j = indZeros  # The population has max individual members in the c code but not here so last item there is zero
        else:
        ...
        if rnd1 <= 0:
            j1 = indZeros  # The population has max individual members in the c code but not here so last item there is zero
        else:
        ...
        if (fit_ptr1 > fit_ptr2):
            pop2_ptr["ind"][k]["genes"][:] = s2_ptr[:];
        elif (fit_ptr1 < fit_ptr2):
            pop2_ptr["ind"][k]["genes"][:] = s1_ptr[:];
        elif (f1_ptr < f2_ptr):
            pop2_ptr["ind"][k]["genes"][:] = s2_ptr[:];
        else:
            pop2_ptr["ind"][k]["genes"][:] = s1_ptr[:];
...
# Crossover------------------------------------------------------------------------
def crossover(new_pop_ptr, mate_pop_ptr, warmup_random, pcross,
              ncross):  # New population changes based on mate population
...
def globpop(popsize, chrom, nchrom, nfunc):
    ...
    popltn["rankar"] = numpy.zeros((2 * popsize, 2 * popsize),
                                   int)  # /*record of array of individual numbers at a particular rank */
    ...
    popltn["fitness"] = numpy.zeros((2 * popsize, nfunc),
                                    float)  # /*Fitness function values for the different	                          # individuals*/
        ...
        while i < (m1):
            if (i == 0 or i == (m1 - 1)):
                ...
                if Diff == 0.0:  # Mehmet: Added in case Diff=0 error
                    Diff = 1e-10
                length[i][1] = abs(
                    fpara1[i + 1][1] - fpara1[i - 1][1]) / Diff;  # // crowding distances are normalized 18.08.2003
            i += 1
        i = 0
        while i < (m1):
            a = length[i][0];
