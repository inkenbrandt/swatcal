...
def SWATparameterChange(TheFileDir, TheValue, IMET, TheLineNo, TheStartSpaceNo, TheSpaceLenght, multiopt="notmulti"):
    ...
    File = open(TheFileDir, "r", encoding='iso-8859-1')  # open the file
    ...
    if IMET == 2 or IMET == 3 or multiopt == "multi":
        ...
        File = open(originalfile, "r", encoding='iso-8859-1')  # open the file
        lines = File.readlines()  # Read lines
        oLines = {}
        for i in range(1, len(lines) + 1):
            oLines[i] = lines[i - 1]
...
# Read model.in
par_ValImet = {}
f = open(modelin, "r", encoding='iso-8859-1')
lines = f.readlines()
for line in lines:
    if line == "\n": break
    columns = line.split("\n")[0].split("\t")
...
# Remove output files
try:
    os.remove("./output.hru"); os.remove("./output.sub")
except:
    pass
